...
def add_cafe_page():
    if request.method == "POST":
        ...
        new_cafe_data = {
            ...
            "map_url": form.get("map_url", "").strip(),
            "img_url": form.get("img_url", "").strip(),
            ...
            "coffee_price": form.get("coffee_price", "").strip() or None,
        }

        new_cafe = Cafe.from_dict(new_cafe_data)
        db.session.add(new_cafe)
        db.session.commit()
...
def delete_cafe_page(cafe_id):
    result = db.session.execute(db.select(Cafe).where(Cafe.id == cafe_id))
    cafe_to_delete = result.scalars().first()
    if cafe_to_delete:
        db.session.delete(cafe_to_delete)
        db.session.commit()
